...
public class SendManufacturingSequenceResponseNode : BTNode
{
    public SendManufacturingSequenceResponseNode() : base("SendManufacturingSequenceResponse") { }

    public override async Task<NodeStatus> Execute()
    {
        var client = Context.Get<MessagingClient>("MessagingClient");
        var resultElement = Context.Get<SubmodelElement>("ManufacturingSequence.Result");
        var negotiation = Context.Get<ProcessChainNegotiationContext>("ProcessChain.Negotiation");
        var success = Context.Get<bool>("ManufacturingSequence.Success");
        var refusalReason = Context.Get<string>("ManufacturingSequence.RefusalReason");

        if (client == null || negotiation == null)
        {
            Logger.LogError("SendManufacturingSequenceResponse: missing client or context");
        ...
        Logger.LogInformation("SendManufacturingSequenceResponse: published {Type} to {Topic}", messageType, topic);
